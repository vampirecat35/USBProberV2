...
- (NSString *) PrintNameForPortAtLocation:(int) port withLocationID:(uint32_t) parentLocationID deviceSpeed:(int)deviceSpeed bandwidth:(UInt32 *)bandwidth
{
    ...
    while ( (usbDeviceRef = IOIteratorNext(matchingServicesIterator)) )
    {
		...
		err = (*iodev)->QueryInterface(iodev, CFUUIDGetUUIDBytes(kIOUSBDeviceInterfaceID942), (LPVOID)&dev);
...
- (int) getPortErrorCount:(IOUSBDeviceInterface942 **) dev port:(int)port
{
...
- (IOReturn) dealWithDevice:(io_service_t) usbDeviceRef
{
	...
	if ( numberObj )
	{
		...
		if ( kr == KERN_SUCCESS)
		{
			...
			{
                if ( IOObjectConformsTo(hubEntry, "AppleUSBXHCIWPT"))
                    break;
        ...
        if ( hubEntry != IO_OBJECT_NULL)
		{
			...
			if ( powerMgrDictRef != NULL )
			{
				...
				CFNumberGetValue((CFNumberRef)current, kCFNumberSInt32Type, &hubCurrentPowerState);
